...
def download(data_dir, filename='cifar-100-python.tar.gz',
...
def fetch(data_dir, filename='cifar-100-python.tar.gz',
          source='http://www.cs.toronto.edu/~kriz/'):
...
def load_images(data_dir, filename):
    path = os.path.join(data_dir, 'cifar-100-batches-py', filename)
...
def load_labels(data_dir, filename):
    ...
    with open(path, 'rb') as f:
        ...
        return to_one_hot(labels, 100)


def load_cifar(base_dir='data', data_dir='cifar100', test=False, flat=True):
